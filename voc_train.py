...
def parse_args():
    ...
    parser.add_argument("--iter", dest="start_iter",
                        ...
                        default=0, type=int)
    parser.add_argument("--threads", dest="threads", default=2, type=int)    
...
# chenjia
# update 2019.01.09
# add for cal the validation map...
def test(db, split, testiter, debug=False, suffix=None):
    # result_dir = system_configs.result_dir
    # result_dir = os.path.join(result_dir, str(testiter), split)
    result_dir = '/data2/chenjia/Detection/voc_cornernet/val_cache'   # 存放validation的test reslut cache file
    ...
    nnet = NetworkFactory(db)    # NetworkFactory class()
    print("loading parameters...")
    nnet.load_params(test_iter)

    test_file = "test.{}".format(db.data)   # test_file: test.coco
...
def train(training_dbs, validation_db, start_iter=0):
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1), file=save_stdout, ncols=20):
            ...
            if iteration % snapshot == 0:    
                ...
                # chenjia
                # model have saved then test the map...
                test(validation_db, 'minival', iteration, debug=False, suffix=None)   # run map in validation datasets
...
if __name__ == "__main__":
    ...
    print ('the config json file: ', cfg_file)
    ...
    train(training_dbs, validation_db, args.start_iter)   # train the net
