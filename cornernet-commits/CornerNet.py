...
def make_tl_layer(dim):   # top left 
    return tl_pool(dim)

def make_br_layer(dim):   # bottom right
    return br_pool(dim)
...
# 搭建CornerNet框架,会调用到kp.py和kp_utils.py...
class model(kp):
...
# pull_weight, push_weight 前的loss系数均为0.1
